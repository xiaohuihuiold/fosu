...
class OSUMapLoader {
  ...
  /// 从路径加载
  Future<OSUMapInfo> _load(String path) async {
    this.path = path;
    File file = File(path);
  ...
  Future<OSUMapInfo> _loadOSB() async {
    if (mapInfo == null) {
      // TODO: 地图未加载
      return null;
    }
  ...
  /// 解包数据
  void _unpackData(String struct, List<String> strings) {
    if (mapInfo == null || struct == null || strings == null) {
      return;
    }
    switch (struct?.trim()) {
      case '[General]':
        Map<String, String> map = _stringToMap(strings);
  ...
  /// 字符串列表转map
  Map<String, String> _stringToMap(List<String> strings) {
...
class _OSUStoryBoardLoader {
  ...
  bool _isHex(List<String> str) {
    ...
    String word = str?.firstWhere((el) => el.contains(reg), orElse: () => null);
    return word != null;
  }
